...
| 2026-09-25 | `configs/ci/charter-ci.yml`: aplicar el endurecimiento de C-006 (D-8) con el mismo texto: guardia en script, raíz de `charter` y archivos de supresión, sin `workflow_dispatch` (o fallar sin `BASE_SHA`), `persist-credentials: false`, `--ignore-gitleaks-allow`, `--disable-nosem`, mínimo 1 test, y fallar si no se detecta ningún stack. Documentar L-1 en GUARDAS | Consejo C-001 | **parcial** (CHANGELOG 2026-09-26): aplicado en `charter-guardia.sh` y GUARDAS (L-1); **falta** en `configs/ci/charter-ci.yml`, que sigue como la versión anterior |
| 2026-09-25 | Tablero: modelar una segunda entrega del probador (`reportes/T-XXX-pruebas.md` con "Entrega 2" pendiente) en "Activar ahora"; leer la **última** fecha de `kit/VERSION` | LIDER (prueba 3) | **parcial** (CHANGELOG 2026-09-26): `pruebas_entregas` en TAREA, CICLO, GUIA y PROBADOR; **falta** en `tablero/tablero.html`, que no lo lee y sigue tomando la primera fecha de `kit/VERSION` |
| 2026-09-25 | `configs/ci/charter-ci.yml`: `continue-on-error` en el **paso** de `npm audit` (o `::warning::`), no en el job. Hoy el check informativo queda en `failure` y el PR limpio muestra una ❌ | LIDER (T-006-r1, hallazgo 1) | **no aplicada**: el CHANGELOG 2026-09-26 la cita, pero `configs/ci/charter-ci.yml` no cambió |
| 2026-09-25 | Guías: advertir que gitleaks y Semgrep en Docker no leen un worktree de Windows (falso verde); usar `-no-color` en actionlint; `${RANGE:+"$RANGE"}` en el job de gitleaks (SC2086) | SENIOR-3 (T-006) | **parcial** (CHANGELOG 2026-09-26): Docker en Windows y `-no-color` en GUARDAS; **falta** `${RANGE:+"$RANGE"}` en `configs/ci/charter-ci.yml` |
| 2026-09-25 | `configs/ci/charter-ci.yml`: fijar las imágenes por versión exacta (hoy `gitleaks:latest` y `semgrep/semgrep` sin etiqueta) y las acciones por SHA con la etiqueta en un comentario, para evitar un cambio silencioso en la cadena de suministro. Aplicado en api-lab con T-006 | PO (prueba 3) | **no aplicada**: el CHANGELOG 2026-09-26 y GUARDAS la citan, pero `configs/ci/charter-ci.yml` sigue con `gitleaks:latest`, `semgrep/semgrep` y `@v4` |

## Para el PO ahora
- **Activar:** nadie
- **Merge pendiente:** ninguno (PR #8–#15 cerrados y ramas borradas, verificado por API)
- **Decisiones pendientes:** completar en el kit maestro `configs/ci/charter-ci.yml` (versión 2) y `tablero/tablero.html` (`pruebas_entregas`, última fecha de VERSION), que el CHANGELOG del 2026-09-26 da por hechos; siguiente paso: seguir con E3 o cerrar la prueba 3
